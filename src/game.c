...
void game_tick_1ms(simon_game_t *game)
{
    ...
    case SIMON_STATE_ATTRACT:
        game->idle_ticks++;
        if ((game->idle_ticks & (IDLE_ANIMATION_SPEED - 1u)) == 0u) {
            hardware_display_idle_animation((uint8_t)(game->idle_ticks >> 6u));
        }
        break;
...
static void apply_command(simon_game_t *game, char value)
{
    ...
    case 's':
        ...
        break;
    case 'q':
        if ((uint32_t)game->playback_delay_ms + PLAYBACK_MANUAL_STEP <= PLAYBACK_DELAY_MAX) {
            game->playback_delay_ms += PLAYBACK_MANUAL_STEP;
            uart_send_delay(game);
        }
        break;
    case 'w':
        if (game->playback_delay_ms > (PLAYBACK_DELAY_MIN + PLAYBACK_MANUAL_STEP)) {
            game->playback_delay_ms -= PLAYBACK_MANUAL_STEP;
            uart_send_delay(game);
        }
        break;
...
void game_handle_uart_char(simon_game_t *game, char value)
{
    if (game->state == SIMON_STATE_NAME_ENTRY) {
        ...
        if (value == 0x08 && game->name_length > 0u) {
            game->name_length--;
            game->name_buffer[game->name_length] = '\0';
...
void game_update_playback_delay(simon_game_t *game, uint16_t pot_value)
{
    ...
    game->playback_delay_ms = (uint16_t)(PLAYBACK_DELAY_MIN + scaled);
}
